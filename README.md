...
1. `새 작업`을 눌러 Codex가 읽을 작업 디렉터리를 한 번 지정합니다. 이후에는 저장된 위치를 자동으로 사용합니다.
2. 해당 디렉터리에서 대화가 생성되면 메시지를 입력하고 전송합니다.
3. 스트리밍되는 응답을 확인합니다.

저장 위치는 사이드바의 `저장 위치 변경`에서 바꿀 수 있습니다. 이전에 저장한 폴더가 삭제되었거나 쓸 수 없으면 이유를 안내하고, 다음 새 작업에서 다시 선택하도록 합니다.
...
1. 대화에서 시작할 작업을 말합니다.
2. 질리언이 작업에 맞는 집중 시간을 제안하고 타이머를 시작할지 확인합니다.
3. 작업명과 시간에 동의하면 질리언이 `start_focus_timer` MCP 도구를 호출합니다.
4. 앱이 요청을 수락하면 집중 화면으로 전환되고 남은 시간과 설정 시간을 표시합니다.

집중 화면과 macOS 상단바는 하나의 표시 상태를 공유해 같은 남은 시간을 보여줍니다. `완료`를 누르면 집중 화면이 즉시 닫히고 대화 흐름에서 회고를 시작합니다. 이미 타이머가 실행 중이면 새 시작 요청은 거절됩니다.
